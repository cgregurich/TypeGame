...
class TypeGame(Tk):
	def __init__(self):
		...
		self.entry.grid(row=0, column=0, padx=LOWER_PADX)
		...
		self.img = None
		self.reset_btn = Button(self.frame_lower, relief=SOLID, command=self.reset)
	...
	def init_restart_button(self):
		...
		self.reset_btn.config(image=self.img)
